...

{
  ...
  echo "scoring $(date)"
  "$PROJ/.venv/bin/python" "$PROJ/excavator/eval_compare.py" --out-dir "$LONG" > "$LONG/RESULTS.txt" 2>&1
  "$PROJ/.venv/bin/python" "$PROJ/excavator/eval_curve.py" --out-dir "$LONG" > "$LONG/CURVE.txt" 2>&1
