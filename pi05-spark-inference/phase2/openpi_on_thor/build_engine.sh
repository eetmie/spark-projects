...
# Auto-detect trtexec (Jetson: /usr/src/tensorrt/bin; nvcr pytorch image: /opt/tensorrt/bin).
TRTEXEC="${TRTEXEC:-$(command -v trtexec || true)}"
for c in /opt/tensorrt/bin/trtexec /usr/src/tensorrt/bin/trtexec; do
    [ -z "$TRTEXEC" ] && [ -e "$c" ] && TRTEXEC="$c"
done
if [ -z "$TRTEXEC" ] || [ ! -e "$TRTEXEC" ]; then
