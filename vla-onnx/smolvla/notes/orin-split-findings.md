...
Running log. Newest first. (Merges the earlier `vla-onnx/smolvla/jetson/notes`.)
...
Re-exported with the `torch.where` fix (see `vla-onnx/smolvla/export_valid_onnx.py`) to kill
...
| ONNX (in the playbook's exports/) | num_steps | nodes | NonZero | BF16 cosine | infer median (Blackwell) |
|----------------------------------|-----------|-------|---------|-------------|--------------------------|
...
the 8 GB Nano. Raw numbers: `vla-onnx/smolvla/precision_sweep_spark.json`.
...
Produced on the Spark (`../../smolvla/`). Two things matter here:
