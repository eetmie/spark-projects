    ...
    for node in model.graph.node:
        try:
            schema = defs.get_schema(
                node.op_type,
                opsets.get(node.domain, opsets.get("", 17)),
                node.domain,
            )
            required = {
                index
                for index, item in enumerate(schema.inputs)
                if item.option != defs.OpSchema.FormalParameterOption.Optional
            }
        except Exception:
            required = set(range(len(node.input)))
        bad.extend(
            (node.op_type, node.name, index)
            for index, value in enumerate(node.input)
            if value == "" and index in required
        )
    if bad:
        raise RuntimeError(f"{path.name}: empty required ONNX inputs: {bad[:4]}")
...
def write_manifest(out: Path) -> int:
    ...
    for path in sorted(out.rglob("*")):
        if (
            ...
            or path.name.startswith("_meta_")
        ):
            continue
